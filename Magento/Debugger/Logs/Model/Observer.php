...
class Debugger_Logs_Model_Observer
{
    ...
    public function captureRequest(Varien_Event_Observer $observer)
    {
        if (Mage::getIsDeveloperMode() === false) {
            return;
        }
        $this->requestTime = (new \DateTimeImmutable())->format('Y-m-d H:i:s');
    }
    ...
    public function captureControllerInfo(Varien_Event_Observer $observer)
    {
        ...
        // récupérer la datetime actuelle
    ...
    protected function initFormattedLog(int $statusCode)
    {
        ...
        if ($this->requestTime) {
            $this->addLogLine("Request Time: " . $this->requestTime->format('Y-m-d H:i:s'));
            $interval = (new \DateTimeImmutable())->diff($this->requestTime);
            $totalMiliseconds = 0;
            ...
            $this->addLogLine("Process Time: " . $totalMiliseconds);
        }
        $request = Mage::app()->getRequest();
        $request->getMethod();
